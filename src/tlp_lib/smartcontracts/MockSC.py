...
class MockSC:
    ...
    def add_solution(self, solution: GCTLP_Encrypted_Message, witness: TLP_Digest):
        solved_puzzle_index = len(self.solutions)

        time = int(datetime.now().timestamp())
        assert time < self.start_time + self.upper_bounds[solved_puzzle_index]
        self.check_solution(self.get_commitment_at(solved_puzzle_index), solution, witness)
